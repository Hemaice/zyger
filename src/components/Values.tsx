...
const Values: React.FC = () => {
  const values = [
    {
      icon: <Lightbulb className="w-16 h-16 text-secondary-500" />,
      title: 'Innovation',
      description: 'Creativity is thinking up new things. Innovation is doing new things.',
      details:
        'We constantly push boundaries, embracing new technologies and methodologies to create solutions that set industry standards.',
    },
    {
      icon: <Award className="w-16 h-16 text-secondary-500" />,
      title: 'Quality',
      description: 'We prioritize excellence in every project.',
      details:
        'Our commitment to quality ensures robust, reliable, and scalable solutions that exceed client expectations and industry standards.',
    },
    {
      icon: <Handshake className="w-16 h-16 text-secondary-500" />,
      title: 'Collaboration',
      description: 'We work closely with clients to understand their vision.',
      details:
        'Partnership is at the heart of our approach, fostering transparent communication and shared success with every client.',
    },
  ];
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-800 mb-6">
            Our{' '}
            <span className="bg-[linear-gradient(135deg,#24E0C0,#3BA6FF,#8A3FFC)] bg-clip-text text-transparent">
              Values
            </span>
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {values.map((value, index) => (
            <div
              key={index}
              className="bg-blue-100 rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 group"
            >
